...
public class TGrafoDirigido implements IGrafoDirigido {
    ...
    /**
     * Devuelve si existe camino entre el origen y destino
     *
     * @param origen
     * @param dest
     * @return
     */
    public boolean existeConexion(Comparable origen, Comparable dest) {
        this.matrizFloyd = this.floyd();
        Object[] verticesArray = vertices.keySet().toArray();
        int indiceY = -1;
        int indiceX = -1;

        // Obtengo [X][Y] de la matriz floy origen destino
        for (int i = 0; i < verticesArray.length; i++) {
            Comparable etiqueta = (Comparable) verticesArray[i];
            if (etiqueta.compareTo(origen) == 0) {
                indiceY = i;
            }
            if (etiqueta.compareTo(dest) == 0) {
                indiceX = i;
            }
        }

        // Luego verifico que esa posicion sea distina de inf.
        if (indiceY != -1 && indiceX != -1) {
            return this.matrizFloyd[indiceY][indiceX] != Double.MAX_VALUE;
        } else {
            return false;
        }
    }
    ...
    public Collection<TVertice> bpf(Comparable etiquetaOrigen) {
        TVertice org = this.getVertices().get(etiquetaOrigen);
        return this.bpf(org);
    }
    ...
    public TCaminos todosLosCaminos(Comparable etiquetaOrigen, Comparable etiquetaDestino) {
        ...
        TVertice destino = this.getVertices().get(etiquetaDestino);
